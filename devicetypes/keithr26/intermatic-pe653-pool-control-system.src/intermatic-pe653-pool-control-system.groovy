...
//		}
  ...
  //      valueTile("fireman", "device.fireman", width: 2, height: 1, inactiveLabel: false, decoration: "flat") {
  //          state "firemanName", label:'${currentValue}', backgroundColor:"#ffffff", action: "setFireman"
  //      }
		...
		standardTile("heaterTile", "device.heater", width: 2, height: 1, canChangeIcon: true) {
            ...
            state "cool",       label: "*COOLING*",  icon: "st.Health & Wellness.health2", backgroundColor: "#233cbc"
		}
...
//    def heaterIndicator = 16
    def getFIREMANSW_84 () { 9 }
    def getFIREMAN_COUNT_84 () { 18 }
    def getFIREMAN_COUNT2_84 () { 18 }
...
//	log.debug "from PE653: ${time1}"
    ...
    // Set Heater indicator (switch 5)
//  condition by heater switch config
    def val1 = ((payload[SWITCHES_84] & heaterIndicator)) ? "on" : "off"
    if (payload[FIREMANSW_84]) {
        val1="cool"
    }
    log.debug "heater ${val1}"
    rslt << createEvent(name: "heater", value: "$val1", isStateChange: true, displayed: true, descriptionText: "Heater is ${val1}")

// set Fireman countdown indicator
    int  secs = (payload[FIREMAN_COUNT_84] & 0xFF) + ((payload[FIREMAN_COUNT_84+1] & 0xFF)<<8)
    int  Fmins  = secs/60
    int  Fsecs  = secs%60
//    def firemanCountdown = "${String.format("%02d",mins)}:${String.format("%02d",secs)}"
    log.debug "countdown1 ${String.format("%02d",Fmins)}"
    log.debug "countdown2 ${String.format("%02d",Fsecs)}"
    log.debug "countdown3 ${String.format("%05d",secs)}"
//    rslt << createEvent(name: "fireman", value: "${firemanCountdown}", displayed: false, descriptionText: "Fireman: ${firemanCountdown}")
...
//    if (debugLevel > "0") {
	log.debug("V1 ManufacturerSpecificReport cmd=${cmd}")
//    }
...
//    if (debugLevel > "0") {
    log.debug("V2 ManufacturerSpecificReport cmd=${cmd}")
//    }
    state.ManufacturerInfo = "ManufacturerInfo:  manufacturerId: $manufacturerId, manufacturerName: $manufacturerName, productId: $productId, productTypeId: $productTypeId"
    createEvent(name: "ManufacturerInfo", value: state.ManufacturerInfo, displayed: true, descriptionText: state.ManufacturerInfo)
}
...
private initUILabels() {
	...
	log.debug "++++InitUILabels ${VSP_ENABLED}"
...
def List refresh() {
    ...
    log.debug "+++++ in refresh ${operationMode2}"
    def cmds = []
...
private List internalConfigure() {
    log.debug "+++++ in internalConfigure ${operationMode2}"
